...
@Component({
  selector: 'app-mum-team',
  templateUrl: './mum-team.component.html',
  styleUrls: ['./mum-team.component.css']
})
export class MUMTeamComponent implements OnInit {
  teamMembers: TeamMember[] = [];
  teamByProject: { [key: string]: TeamMember[] } = {};
  filteredMembers: TeamMember[] = [];
  locations: string[] = ['Mumbai', 'Coimbatore', 'Chennai', 'Bangalore', 'Hyderabad', 'Pune', 'Delhi', 'Ahmedabad', 'Bhubaneshwar', 'Kolkata', 'Thane'];
  locProjects: string[] = [];
  ...
  constructor(private teamService: TeamService) {}
     ...
     ngOnInit(): any {
    // console.log("get data" + (this.teamService.getTeamData().subscribe(data => console.log(JSON.stringify(data)))) )
      ...
      this.locProjects = this.getProjectsForLocation(data, 'Mumbai');
      this.locTeamMembers = this.getTeamMembersForLocation(data, 'Mumbai');
  ...
  // getMembersForEmail(email: string): TeamMember[] {
  //   return this.locTeamMembers.filter(member => member.projects.includes(email));
  // }
